        ...
        for k in K:
            for w in waiting_counter_max:
                ...
                for t in Tau:
                    ...
                    #waiting_vector_file =  f'{network_root}/{network_type}/{n}/{k}/{t}/waiting_vector.csv'
                    polarization_file = f'{network_root}/{network_type}/{n}/{k}/{w}/{t}/polarization.csv'
                    ## If the network_path does not exist it is created
                    if not os.path.exists(f'{network_root}/{network_type}/{n}/{k}/{w}/{t}'):
                        os.makedirs(f'{network_root}/{network_type}/{n}/{k}/{w}/{t}')
                    ...
                    for graph_path in list(get_recursive_graph_paths(f'Networks/{network_type}/{n}/{k}'))[:]:
                        ...
                        percent = int(fraction_of_seeds * n)
                        if percent > 0:
                        ...
                        with open(f'{network_root}/{network_type}/{n}/{k}/{w}/{t}/infected_p={G.gp.ID}.json', "w") as f:
                            json.dump(data_json, f, indent=2)
                        with open(f'{network_root}/{network_type}/{n}/{k}/{w}/{t}/waiting_dct_p={G.gp.ID}.json', 'w') as f:
