...
class LTM_memory:
    def __init__(self,network_root,network_class,network_type,N,K,Tau,Persuasion,cascades,threshold,probabilities,desired_realizations,unique_network_seeds,cols,b):
        ...
        self.desired_realizations = desired_realizations
        self.unique_network_seeds = unique_network_seeds
        self.cols = cols
        self.b = b
            ...
            for k in self.K:
                for t in self.Tau:
                    for pers in self.Persuasion:
                        ...
                        count = 0
                        nets_prop_file =  f'{self.network_root}/{self.network_type}/{n}/{k}/{t}/{pers}/props.csv'
                        polarization_file = f'{self.network_root}/{self.network_type}/{n}/{k}/{t}/{pers}/polarization.csv'
                        ## If the network_path does not exist it is created
                        if not os.path.exists(f'{self.network_root}/{self.network_type}/{n}/{k}/{t}/{pers}'):
                            os.makedirs(f'{self.network_root}/{self.network_type}/{n}/{k}/{t}/{pers}')
                            
                        if not os.path.exists(f'{self.network_root}/{self.network_type}/Networks/n={n}/k={k}'):
                            os.makedirs(f'{self.network_root}/{self.network_type}/Networks/n={n}/k={k}')
                            
                        config_path = f'{self.network_root}/{self.network_type}/{n}/{k}/{t}/{pers}/config.ini'

                        config = configparser.ConfigParser()

                        config['Configuration'] = {'cascade': self.cascades, 'threshold' : self.threshold, 'probabilities' : self.probabilities, 'Alphas' : Alphas, 'Nodes' : n, 'Neighbors' : k, 'Memory' : t, 'Network type' : self.network_type, 'Persuasion' : pers}
                        ...
                        realization_counter=dict.fromkeys(self.probabilities,0)
                        
                        # Count the graphs in network path and create missing networks according to  parameters (N,k,p)
                        for graph_path in get_recursive_graph_paths(f'{self.network_root}/{self.network_type}/Networks/n={n}/k={k}'):
                        ...
                        for p in realization_counter:
                            ...
                            # Create new networks, until the disired number of realizations is achieved
                                ...
                                while realization_counter[p] < self.desired_realizations:
                                    ...
                                    G.graph_properties['ntype'] = G.new_graph_property('string',val=self.network_type)
                                    ...
                                    G.save(f'{self.network_root}/{self.network_type}/Networks/n={n}/k={k}/{G.gp.ID}.gt')
                                    realization_counter[p] += 1

                        for graph_path in list(get_recursive_graph_paths(f'{self.network_root}/{self.network_type}/Networks/n={n}/k={k}'))[:]:
            ...
            for neighbor_k in self.K:
                for tau in self.Tau:
                    for pers in self.Persuasion:
                        ...
                        nets_prop_file =  f'{self.network_root}/{self.network_type}/{n_nodes}/{neighbor_k}/{tau}/{pers}/props.csv'
                        polarization_file = f'{self.network_root}/{self.network_type}/{n_nodes}/{neighbor_k}/{tau}/{pers}/polarization.csv'
                        ...
                        flag_l_label=0
                        for network in self.network_class:
