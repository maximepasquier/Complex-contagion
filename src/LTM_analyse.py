    ...
    for n_nodes in N:
        for neighbor_k in K:
            for w in waiting_counter_max:
                ...
                nets_prop_file =  f'{network_root}/{network_type}/{n_nodes}/{neighbor_k}/{w}/{0}/props.csv'
                base_polarization_file = f'{network_root}/{network_type}/{n_nodes}/{neighbor_k}/{w}/{0}/polarization.csv'
                ...
                for cas in cascades[:]:
                    for idx,p in enumerate(probabilities[::-1]):
                        ...
                        for tau in Tau:
                            ...
                            eval_polarization_file = f'{network_root}/{network_type}/{n_nodes}/{neighbor_k}/{w}/{tau}/polarization.csv'
                        ...
                        for th in base_polarization.index.get_level_values(3).unique():
                            (network_corr.loc[('CC','p',th),'speraman'],network_corr.loc[('T','p',th),'speraman'],network_corr.loc[('SP','p',th),'speraman'],network_corr.loc[('Rg','p',th),'speraman'])   = network_props.corrwith((polarization_mean.loc[ix[:,network_type,:,th],f'{cas}'].dropna().groupby('ID').mean()),method=methods)[['CC','T','SP','Rg']]


                        for idx,param in enumerate(network_corr.index.get_level_values(0).unique()):
                            axs.plot(network_corr.loc[ix[param,:,:],:].index.get_level_values(2),network_corr.loc[ix[param,:,:],:],label=corr_fig_legend_label[idx],marker=['<','^','>','^'][idx%5],zorder=[3,2,1,4,5][idx%5],c=colors[idx%5],linewidth=2,markersize=[5,5,5,5][idx%5],ls=[':','--','-','--'][idx%5],markeredgewidth=[1,1,1,1][idx%5],markerfacecolor='none')

                        ## set scale stuff
                        axs.set_ylabel(r'Correlation $r_s(v,\chi)$',labelpad=2.5)
                        axs.set_xlabel(r'Threshold $( \theta )$',labelpad=2)
                        axs.set_ylim([-1.05,1.475])
                        axs.set_xlim([-0.02,0.51])

                        axs.text(-0.02,1.03,r'\textbf{(a)}',transform=axs.transAxes,fontsize=8)
                        axs.text(-0.02,1.03,r'\textbf{(b)}',transform=axs.transAxes,fontsize=8)
                        legend1 = axs.legend(title=r'$\chi$',loc=[1.1,0],borderpad=0.2,markerscale=1,handlelength=2,handletextpad=0.4,fontsize=7)
                        ...
                        if save:
                            fig_path = f'figs/analyse/{network_type}/{n_nodes}/{neighbor_k}/{w}'
