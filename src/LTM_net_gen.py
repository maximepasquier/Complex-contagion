    ...
    for k in K:
        for t in Tau:
            for pers in Persuasion:
                ...
                config['Configuration'] = {'cascade': cascades, 'threshold' : threshold, 'probabilities' : probabilities, 'Alphas' : Alphas, 'Nodes' : n, 'Neighbors' : k, 'Memory' : t, 'Network type' : network_type, 'Persuasion' : pers}
                    ...
                    for seed in seed_nodes:
                        if(t == 0): # no memory used
                            if(pers == 0): # no persuasion
                                ...
                                infected_vectormap, selected_seed, _ = linear_threshold_persuasion_model(G,threshold,pers,seed_nodes=[seed])
                        else: # memory used
                            #* Modèle d'inertie
